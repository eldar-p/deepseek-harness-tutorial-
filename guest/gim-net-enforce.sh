#!/bin/bash
# gim-net-enforce — guest egress policy from GIM_NET_* (beta).
# Requires: CAP_NET_ADMIN, iptables, dig (dnsutils). Failures are non-fatal.
set -u

MODE="${GIM_NET_MODE:-allowlist}"
ALLOW="${GIM_NET_ALLOWLIST:-}"
...
apply_proxy() {
  local proxy_host="${GIM_PROXY_HOST:-host.docker.internal}"
  local proxy_port="${GIM_PROXY_PORT:-3128}"
