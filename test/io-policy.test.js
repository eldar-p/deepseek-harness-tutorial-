...
import { rotateLogIfLarge, cleanStalePartFiles, DEEP_LOG_MAX_BYTES } from '../src/io-policy.js'

test('rotateLogIfLarge rotates when over cap', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'deep-io-'))
  const log = path.join(dir, 'deep.log')
  fs.writeFileSync(log, 'x'.repeat(DEEP_LOG_MAX_BYTES + 100))
  assert.equal(rotateLogIfLarge(log, { maxBytes: 100 }), true)
  assert.ok(fs.existsSync(path.join(dir, 'deep.log.1')))
  fs.rmSync(dir, { recursive: true })
})

test('cleanStalePartFiles removes old part files', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'deep-part-'))
