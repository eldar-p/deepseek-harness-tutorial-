# Deep CLI — privacy & telemetry

- **No phone-home telemetry** in the CLI. `telemetry: false` in default config.
- **deep.log** records events only (`event=…`), never prompt or chat bodies.
...
Opt-in only: set `telemetry: true` in `~/.deep/config.json` when a future release documents what is sent.
