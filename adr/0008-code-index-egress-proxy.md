...
## Context
Deep agent relied on manual file reads and grep. Guest container received network policy via iptables only; API secrets could leak via env vars. Competitors (Aide, Continue, Cline, agent-sandbox) use semantic indexing and sidecar egress.
...
- CLI: `deep index build|search|status`
...
- `src/egress-proxy.js` — allowlist forward proxy; reads `~/.deep/secrets.json` on host only
- Guest gets `HTTP_PROXY=http://host.docker.internal:<port>` — never sees secrets file
- iptables in guest remain as defense-in-depth (`DEEP_NET_MODE=proxy`)
- Secret header injection applies to plain HTTP requests; HTTPS CONNECT is allowlist tunnel only

## Consequences
- Large repos: agent should `deep index search` before bulk reads
- Users edit `~/.deep/secrets.template.json` → `secrets.json` (mode 600)
